...
SEASTAR_TEST_CASE(system_clock_sanity) {
    ...
    //
    // Check two out of three samples in order to account for the low probability that the change between the two
    // clocks happens to span the boundary between seconds.
    //

    std::array<bool, 3> const attempts{{check_matching(), check_matching(), check_matching()}};
    auto const success_count = std::count(attempts.cbegin(), attempts.cend(), true);
