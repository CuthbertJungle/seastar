...
public:
    ...
    static steady_time_point steady_now() {
        auto const nr = _steady_now.load(std::memory_order_relaxed);
        return steady_time_point(steady_duration(nr));
    }

    static system_time_point system_now() {
        auto const nr = _system_now.load(std::memory_order_relaxed);
...
private:
    ...
    alignas(64) static std::atomic<steady_rep> _steady_now;
